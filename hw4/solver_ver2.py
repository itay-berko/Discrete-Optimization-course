...
def define_cons1(model, x, y, facilities, customers):
    for c in range(len(customers)):
        for f in range(len(facilities)):
            con = pl.LpConstraint(pl.LpAffineExpression(y[(f, c)] <= x[f]), sense=-1, rhs=0)
            model += con


def define_cons2(model, y, facilities, customers):
    for c in range(len(customers)):
        con = pl.LpConstraint(pl.LpAffineExpression(lpSum(y[(f, c)] for f in range(len(facilities))) == 1),
                              sense=0, rhs=0)
...
def define_cons3(model, y, facilities, customers):
    for f in range(len(facilities)):
        ...
        con = pl.LpConstraint(pl.LpAffineExpression(lpSum(y[(f, c)] * customers[c].demand for c in range(len(customers))) <= curr_capacity),
                              sense=-1, rhs=0)
        model += con
