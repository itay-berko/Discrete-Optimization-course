...
# heuristics search best neighbor
    ...
    while iter_counter < max_iter:
        ...
        for i in range(len(curr_opt_seq)):
            ...
            if curr_fun_val < initial_fun_val:
                first_neighbors = first_neighbors.append({'index': i, 'seq': curr_seq, 'fun_val': curr_fun_val}, ignore_index=True)
                neighbors_count += 1
...
# random walk
def local_search_opt2(dis_matrix, initial_sequence):
    # searching all the point, and chose the best one to start with
    # initial with the first point in the sequence
    point = initial_sequence[0]
    edge_dis = dis_matrix[initial_sequence[0]][initial_sequence[1]]
    opt_seq = K_OPT(initial_sequence, dis_matrix, 0, edge_dis)
    if opt_seq is None:
    ...
    for i in range(1, len(initial_sequence)):

        if i == len(initial_sequence) - 1:
            edge_dis = dis_matrix[initial_sequence[i]][initial_sequence[0]]
        else:
            edge_dis = dis_matrix[initial_sequence[i]][initial_sequence[i + 1]]

        temp_seq = K_OPT(initial_sequence, dis_matrix, i, edge_dis)
        if temp_seq is None:
            continue
        else:
            temp_fun_val = calc_fun_value(temp_seq, dis_matrix)

        if temp_fun_val < min_fun_val:
            opt_seq = temp_seq
            min_fun_val = temp_fun_val
            point = initial_sequence[i]

    start_origin_index = update_index(point, opt_seq)
...
def metropolis_heuristic(dis_matrix, initial_sequence):
    # start improve the  seq with the longest edge
    start_origin_index, max_dis = get_max_edge(dis_matrix, initial_sequence)
    ...
    while iter_counter < max_iter:
        ...
        if curr_seq is not None:
            ...
            if curr_fun_val < min_fun_val:
                ...
                form = 1 / (np.exp(curr_fun_val - min_fun_val) / T0)
                if rand1 <= form:
                    start_origin_index = update_index(opt_seq[i], curr_seq)
                    opt_seq = curr_seq
                    min_fun_val = curr_fun_val
...
def metropolis_heuristic_ver2(dis_matrix, initial_sequence):
    # searching all the point, and chose the best one to start with
    # initial with the first point in the sequence
    point = initial_sequence[0]
    edge_dis = dis_matrix[initial_sequence[0]][initial_sequence[1]]
...
def solve_it(input_data):
    ...
    opt_seq1 = local_search_opt1(dis_matrix, initial_sequence)
