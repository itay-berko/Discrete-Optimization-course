...
def tabu_search(dis_matrix, initial_sequence, not_visited):
    ...
    tabu_counter = 1
    tabu_seq = []
    tabu_search.append(opt_seq)
    max_tabu = 5


    while iter_counter < max_iter:

        for i in range(len(opt_seq)):
        ...
        start_origin_index = update_index(opt_seq[neighbor[0]], neighbor[1])
        curr_opt_seq = neighbor[1]
        min_fun_val = neighbor[2]
...
def multi_local_search(dis_matrix, initial_sequence):

    # initial solution
    not_visited = initial_sequence.copy()
    opt_seq = local_search_opt1(dis_matrix, initial_sequence, not_visited)
    if opt_seq is None:
        opt_seq = initial_sequence
        min_fun_val = calc_fun_value(opt_seq, dis_matrix)
    else:
    ...
    for i in range(iteration):
        ...
        curr_seq = local_search_opt1(dis_matrix, initial_sequence, not_visited)
        if curr_seq is None:
            continue
...
def solve_it(input_data):
    ...
    opt_seq = multi_local_search(dis_matrix, initial_sequence)
